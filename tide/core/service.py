...
class StackService:
    ...
    def infer_graph(self) -> StackGraph:
        ...
        for branch in sorted(local_names):
            upstream_parent = self.repo.upstream_branch_name(branch)
            if upstream_parent is not None and upstream_parent in names:
                remote_parent[branch] = upstream_parent
            hint = self.repo.branch_parent_hint(branch)
            if hint is not None and hint in names and branch not in remote_parent:
                remote_parent[branch] = hint
        ...
        remote_edges = sorted(remote_parent.items())
    ...
    def create_missing_prs(
        ...
        branches: list[str],
        draft: bool = True,
        title_template: str = "$HEAD",
        body_template: str = "Auto-generated by tide for branch $HEAD",
    ) -> list[PullRequest]:
    ...
    def branch_for_pr_number(self, number: int, names: list[str]) -> str:
        for pr in self.forge.list_prs_sync(names):
            if pr.number == number:
                return pr.head
        raise InputError(f"unknown PR number: {number}")

    def _render_pr_template(self, *, template: str, head: str, base: str) -> str:
        rendered = template.replace("$HEAD", head).replace("$BASE", base)
        return rendered
