...
def _render_show(obj: CliContext) -> str:
    ...
    for branch in sorted(graph.nodes):
        upstream = obj.repo.branch_upstream(branch)
        if upstream is None:
            continue
        try:
            divergence[branch] = obj.repo.ahead_behind(branch, upstream)
        except TideError:
            continue
    if obj.json_output:
        ...
        for node in payload["nodes"]:
            ...
            ahead, behind = divergence.get(node, (0, 0))
    ...
    def node_line(node: str, marker: str) -> str:
        ...
        if node in prs:
            ...
            if prs[node].checks_summary:
                suffix_parts.append(f"checks={prs[node].checks_summary}")
            if prs[node].review_summary:
                suffix_parts.append(f"reviews={prs[node].review_summary}")
        if node == current:
            suffix_parts.append("current")
        if node in divergence:
            ahead, behind = divergence[node]
            if ahead or behind:
...
def pr_create_cmd(
    ...
    def run() -> None:
        graph = obj.service.infer_graph()
        selector = stack_selector
        if head_pr is not None:
            selector = obj.service.branch_for_pr_number(head_pr, list(graph.nodes.keys()))
        branches = obj.service.resolve_scope(graph, selector, scope)
        github_cfg = obj.config.values.get("forge", {}).get("github", {})
        title_template = str(github_cfg.get("title_template", "$HEAD"))
        body_template = str(
            github_cfg.get("body_template", "Auto-generated by tide for branch $HEAD")
        )
        created = obj.service.create_missing_prs(
            graph,
            branches,
            draft=draft,
